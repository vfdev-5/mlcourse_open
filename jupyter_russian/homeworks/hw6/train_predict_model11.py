...
def compute_timebased_features(df):
    ...
    df.loc[:, 'yearmonth'] = df.loc[:, 'time1'].apply(lambda x: -(x.year * 100 + x.month) / 201500.0)
...
print("- CV model")
forest_params = {
    'n_estimators': range(18, 20, 2),
    'max_depth': range(9, 10),
    'max_features': range(80, 90, 5),
    'class_weight': [
        {0: 1, 1: 151.1},       
        {0: 1, 1: 151.2},
        {0: 1, 1: 151.3},        
...
gs = GridSearchCV(rf, forest_params, scoring='roc_auc', cv=tscv, n_jobs=-1, verbose=True)
gs.fit(X, y)

print(gs.best_score_, gs.best_params_, gs.cv_results_['std_test_score'][gs.best_index_])
...
print("- Predictions")
X_test = test_sites_tfidf
X_test = model.transform(X_test)
X_test = add_timebased_features(X_test, test_df)
...
write_to_submission_file(test_probas, "assignment6_alice_submission_model11_roc_auc={:.4f}_FN={}_FP={}_TP={}.csv"
