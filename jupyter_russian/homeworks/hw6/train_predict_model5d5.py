...
n_splits = 7
max_train_size = None
vectorizer1 = CountVectorizer(max_features=5000000, ngram_range=(1, 3))
vectorizer2 = TfidfVectorizer(max_features=5000000, ngram_range=(1, 6))
...
vectorizer1.fit(sites_full_aligned)
vectorizer2.fit(sites_full_aligned)

train_sites_tfidf = vectorizer1.transform(sites_full_aligned[train_indices])
test_sites_tfidf = vectorizer1.transform(sites_full_aligned[test_indices])

train_sites_cvec = vectorizer2.transform(sites_full_aligned[train_indices])
test_sites_cvec = vectorizer2.transform(sites_full_aligned[test_indices])
...
def compute_timebased_features(df):
    ...
    df.loc[:, 'yearmonth'] = df.loc[:, 'time1'].apply(lambda x: -(x.year * 100 + x.month) / 201500.0 - 0.5)
    ...
    df.loc[:, 'hm'] = df.loc[:, 'time1'].apply(lambda x: (x.hour * 100 + x.minute) / 2400.0 - 0.5)
...
X = hstack([train_sites_tfidf, train_sites_cvec])
X = add_timebased_features(X, train_df)
print(X.shape)
...
print("- Predictions")
X_test = hstack([test_sites_tfidf, test_sites_cvec])
X = add_timebased_features(X, test_df)
...
write_to_submission_file(test_probas, "assignment6_alice_submission_model5d5_roc_auc={:.4f}_FN={}_FP={}_TP={}.csv"
