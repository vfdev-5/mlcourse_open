...
X_train = compute_features(train_df, train_sites_sparse, is_train=True)
log_reg_cv, score, best_C = get_auc_lr_cv(X_train, y, max_iter=max_iter,
...
X_test = X_train = compute_features(test_df, test_sites_sparse, is_train=False)
y_test = log_reg.predict_proba(X_test)[:, 1]

score = np.max(score)
write_to_submission_file(y_test, "assignment6_alice_submission_model_L4_roc_auc={:.4f}_FN={}_FP={}_TP={}.csv"
