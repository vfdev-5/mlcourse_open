...
def compute_timebased_features(df):
    ...
    #     res1 = pd.get_dummies(df['daytime1'])

    df.loc[:, 'h'] = df.loc[:, 'time1'].apply(lambda x: x.hour)
    #     res2 = pd.get_dummies(df['h'])
    ...
    features = ['session_duration', 'daytime1', 'is_not_saturday', 'is_not_sunday', 'h', 'wd']
    # return pd.concat([df[features], res, res1], axis=1)
    return df[features]
...
write_to_submission_file(test_probas, "assignment6_alice_submission_model5e_FN={}_FP={}_TP={}.csv"
